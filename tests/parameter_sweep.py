...
def main():
    ...
    parser.add_argument("--source", default="csv", choices=["auto", "alphavantage", "csv", "yfinance", "pykrx"])
    parser.add_argument("--csv-path", default=None)
    parser.add_argument("--rsi-oversold-list", default="30,35,40,45")
    parser.add_argument("--rsi-overbought-list", default="65,70,75")
    parser.add_argument("--ma-short-list", default="5,10,20")
    parser.add_argument("--ma-long-list", default="20,40,60")
    parser.add_argument("--bb-period-list", default="10,20")
    parser.add_argument("--bb-std-dev-list", default="1.5,2.0")
    ...
    bb_std_dev_values = parse_float_list(args.bb_std_dev_list)

    total_combos = (
        len(rsi_oversold_values) * len(rsi_overbought_values)
        * len(ma_short_values) * len(ma_long_values)
        * len(bb_period_values) * len(bb_std_dev_values)
    )
    print(f"총 {total_combos}개 조합 테스트 시작...")

    rows = []
    for rsi_oversold, rsi_overbought, ma_short, ma_long, bb_period, bb_std_dev in product(
        rsi_oversold_values,
